...
9. **슬라이드 제목을 SVG 안에 넣지 마라.** `deck.diagram_svg()` 가 슬라이드 위에 제목과 밑줄을 따로 그린다.
   그림 안 첫 글자는 패널 머리글(`① …` 같은 소제목)이어야 하고, 슬라이드 제목과 같은 문장이면 안 된다.
   같은 이유로 그림 아래 띠에 쓴 문장을 `callout=` 에 옮겨 적지 마라 — 한 슬라이드에 같은 말이 두 번 찍힌다(L-18).
   `check_layout.py` 가 유사도 0.80 이상이면 실패시킨다.
...
**설명 불릿을 함께 놓는 슬라이드라면 H 를 400 이하로 잡아라.** 실측: H 515 → 6.1pt, H 510 → 6.3pt, H 356 → 8.6pt.
이미 그린 뒤라면 불릿을 줄여 봐야 소용없다 — `diagram_svg` 는 items 가 하나라도 있으면 0.80in 을 떼 간다.
그때는 items 를 통째로 비우고 그 내용을 발표자 노트로 옮겨야 그림이 커진다(L-16).
