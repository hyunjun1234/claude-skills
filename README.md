...
| `hooks/dd_artifact.sh` | PostToolUse | 이 세션이 실제로 그림·pptx 를 냈는지 표시한다(약한 표현을 판정하는 근거) |
| `hooks/dd_guard.sh` | Stop | 교훈을 안 남기고 끝내려 하면 **수정 요청 한 건당 한 번** 막는다 |
| `hooks/dd_sync.sh` | Stop (비동기) | 스킬 저장소 변경을 자동으로 커밋·푸시한다 |

감지는 세 단계로 나눠 오탐을 줄인다.

- **강신호**(`겹쳐` `넘쳐` `잘렸` `고쳐` `복구` …) — 그 자체로 수정 요청이다.
- **약신호**(`위치` `크게` `색을` `말고` …) — 이것만으로는 모른다.
  **이 세션이 실제로 그림을 낸 뒤**에만 인정한다.
- **생성**(`만들어줘` `그려줘` …) — 강신호가 없으면 수정이 아니라 새로 만드는 것이다. 발동하지 않는다.

또한 스킬·코드 이야기(`스킬` `훅` `.py` `커밋` …)와 질문(`어디` `뭐야` `?`)은 제외하고,
빈 줄로 나뉜 **문단** 안에서만 결합을 인정한다(붙여넣은 로그의 경로가 딴 문장과 엮이지 않게).
